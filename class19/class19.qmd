...
Pertissis is a bacterial lung infection also known as Whooping cough. Let's 
begin by examining CDC reported case numbers in the US:
...
The full introduction of mandatory wP (whole-cell) Pertussis immunization in 
the mid 1940s lead to a dramatic reduction in case numbers (from over 200,000 to
100s).
...
They make their data available via JSON format API endpoints - basically the 
database tables in a key:value type format like "infancy_vac":"wP". To read 
this we can use the `read_json()` function from the **jsonlite** package.
...
This breakdown is not particulary representative of the US population - this is a serious cavet for this study. However, it is still the largest smaple of it's type every assembled.
...
to analyze this data and know who a give Ab sample was colected for and when.
...
Now let's join the `ab_titer` table with our `meta` table so we have all informations about a given Ab measurment
...
IgG is crucial for long-term immunity and responding to bacterial infections
...
This plot shows the time course of Pertussis toxin (PT) antibody responses for a large set of wP (teal color) and aP (red color) individuals. Levels peak at day 14 and are larger in magnitute for wP than aP individuals.

There are lots of cool things to explore in this dataset and we need coding and biology knoweledge to do it effectively - i.e. you gus!!!
