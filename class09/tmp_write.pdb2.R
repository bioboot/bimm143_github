write.pdb2 <- 
  function (pdb = NULL, file = "R.pdb", xyz = pdb$xyz, type = NULL, 
    resno = NULL, resid = NULL, eleno = NULL, elety = NULL, chain = NULL, 
    insert = NULL, alt = NULL, o = NULL, b = NULL, segid = NULL, 
    elesy = NULL, charge = NULL, append = FALSE, verbose = FALSE, 
    chainter = FALSE, end = TRUE, sse = FALSE, print.segid = FALSE) 
{
    ...
    check.lengths <- sum(length(card), length(resno), length(resid), 
        length(eleno), length(elety), length(chain), length(insert), 
        length(alt), length(o), length(b), length(segid), length(elesy), 
        length(charge))
...
#        format <- "%-6s%5s  %-3s%1s%3s %1s%4s%1s%3s%8.3f%8.3f%8.3f%6.2f%6.2f%6s%-4s%2s%2s"
         format <- "%-6s%5s  %-3s%1s%3s %1s%4s%1s%3s%8.3f%8.3f%8.3f%6.2f%6.2f%6s%-4s%2s%2s"
      
              if (nchar(elety) > 3 || (is.character(elesy) && nchar(elesy) > 
            1)) {
           #format <- "%-6s%5s %-4s%1s%3s %1s%4s%1s%3s%8.3f%8.3f%8.3f%6.2f%6.2f%6s%-4s%2s%2s"
            format <- "%-6s%5s %-4s%1s%3s %1s%4s%1s%3s%8.3f%8.3f%8.3f%6.2f%6.2f%6s%-4s%2s%2s"
              }
        sprintf(format, card, eleno, elety, alt, resid, chain, 
            resno, insert, "", x, y, z, o, b, "", segid, elesy, 
            charge)
    }
    format.sse <- function(pdb) {
        lapply(c(helix = "helix", sheet = "sheet"), function(x) {
            ...
            if (length(sse$start) > 0) {
                ref <- pdb$atom[pdb$calpha, c("resno", "chain", 
                  "insert", "resid")]
                refkey <- paste(ref$resno, ref$chain, ref$insert, 
                  sep = "_")
                ...
                resid = data.frame(start = ref$resid[match(skey, 
                  refkey)], end = ref$resid[match(ekey, refkey)])
                insert = data.frame(start = names(sse$start), 
                  end = names(sse$end))
                type = ifelse(x == "helix", "type", "sense")
                length = match(ekey, refkey) - match(skey, refkey) + 
                  1
                return(list(resno = resno, resid = resid, chain = sse$chain, 
                  insert = insert, type = sse[[type]], length = length))
            }
            else {
                return(NULL)
            }
    ...
    if (!is.null(pdb) && sse) {
        ...
        if (!is.null(new.sse$helix)) {
            ...
            for (i in 1:nrow(xx$resno)) lines <- rbind(lines, 
                sprintf(format, "HELIX", i, i, xx$resid$start[i], 
        ...
        if (!is.null(new.sse$sheet)) {
            ...
            for (i in 1:nrow(xx$resno)) lines <- rbind(lines, 
                sprintf(format, "SHEET", i, "S1", nrow(xx$resno), 
    ...
    if (nfile == 1) {
        ...
        for (i in 1:natom) {
            lines <- rbind(lines, atom.print(card = card[i], 
                eleno = as.character(teleno[i] + ii), elety = elety[i], 
                alt = alt[i], resid = resid[i], chain = chain[i], 
                resno = resno[i], insert = insert[i], x = coords[i, 
                  1], y = coords[i, 2], z = coords[i, 3], o = o[i], 
                b = b[i], segid = segid[i], elesy = elesy[i], 
                charge = charge[i]))
            if (chainter) {
                if (i %in% ter.lines) {
                  ii = ii + 1
                  lines <- rbind(lines, sprintf("%-6s%5s%6s%3s%1s%1s%4s%1s", 
                    ...
                    resid[i], "", chain[i], resno[i], insert[i]))
                }
            }
        }
        write.table(lines, file = file, quote = FALSE, row.names = FALSE, 
        ...
        if (chainter) {
            ...
            cat(sprintf("%-6s%5s%6s%3s%1s%1s%4s%1s\n", "TER", 
                as.character(teleno[i] + ii), "", resid[i], "", 
                chain[i], resno[i], insert[i]), file = file, 
                append = TRUE)
        }
        if (end) {
        ...
        if (!append) 
            unlink(file)
        for (j in 1:nfile) {
            coords <- matrix(round(as.numeric(xyz[j, ]), 3), 
                ncol = 3, byrow = TRUE)
            lines <- NULL
            ii = 0
            teleno <- as.numeric(eleno)
            for (i in 1:natom) {
                lines <- rbind(lines, atom.print(eleno = as.character(teleno[i] + 
                  ii), elety = elety[i], alt = alt[i], resid = resid[i], 
                  chain = chain[i], resno = resno[i], insert = insert[i], 
                  x = coords[i, 1], y = coords[i, 2], z = coords[i, 
                    3], o = o[i], b = b[i], segid = segid[i], 
                  elesy = elesy[i], charge = charge[i]))
                if (chainter) {
                  if (i %in% ter.lines) {
                    ii = ii + 1
                    lines <- rbind(lines, sprintf("%-6s%5s%6s%3s%1s%1s%4s%1s", 
